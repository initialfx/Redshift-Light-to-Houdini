# Export Environment(Dome) Light from Maya
...
def write_json():
    attributes = ['translateX','translateY','translateZ','rotateX','rotateY','rotateZ','scaleX','scaleY','scaleZ','tex0','exposure0','envType','samples','srgbToLinear0','color','saturation0','gamma0',
'diffuseRayContributionScale','glossyRayContributionScale','singleScatteringRayContributionScale','multipleScatteringRayContributionScale','indirectRayContributionScale','background_enable']


    #attr = json.dumps(attribute_generator(attributes, list_all_lamps()))
    attr = json.dumps(attribute_generator(attributes, list_all_lamps()), sort_keys=True, indent=4)
    return attr
